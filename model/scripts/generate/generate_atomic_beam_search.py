...
eval_file_name = eval_file_name[:-7] + "/{}.json".format(split)
eval_file_name = eval_file_name.replace("models/", "results/gens/")
f = open("human-eval-events_dev.txt", "w")
f.close()
...
with torch.no_grad():
    for idx in tqdm(total):
        ...
        logits = lm_model(XMB.unsqueeze(1), attention_mask=MMB)
        logits = logits[:, -1, :].contiguous()
        dist = lm_probs[:, -1, :].contiguous()   #.squeeze() 
        #targets = targets.repeat(10)
        l =  F.nll_loss(dist.view(-1, dist.size(-1)), targets)  #loss_fct(dist.view(-1, dist.size(-1)), targets)
        dist = lm_probs[:, -1, :].squeeze() 
        #targets = targets.repeat(10)
        l =  loss_fct(logits.view(-1, logits.size(-1)), targets)  #loss_fct(dist.view(-1, dist.size(-1)), targets)
        ...
        for _ in range(args.gen_len):
           # Compute distribution for current beam
            ...
            dist = lm_probs[:, -1, :].squeeze()
            #if target_counter < 41:
            #    target_counter = target_counter + 1
            #    generate_counter = generate_counter + 1
            #    targets = Ref[:, target_counter].contiguous().view(-1)
            #    targets = targets.repeat(2)
            #    l = F.nll_loss(dist.view(-1, dist.size(-1)), targets, reduction="none")               
            #    loss.append(l)
            #else: 
            #   targets = Ref[:, target_counter].contiguous().view(-1)
            #   l = loss_fct(dist.view(-1, dist.size(-1)), targets)
            #   generate_counter = generate_counter + 1
            #   loss = loss + l
        ...
        #avg = sum(loss[:len(beam_losses)])/len(beam_losses)
        
        print(target_length)
        print(loss)
        print("Seq length:",len(beam_seqs))
        print(beam_losses)
        sequence_all['beams'] = beams
        print(beams)
        f = open("atomic_generate.txt", "a")
        f.write(str(sequence_all)+'\n')
        f.close()
        #f1 = open("anli_forward_gpt2_loss.txt", "a")
        #f1.write(str(avg)+'\n')
        #f1.close()
...
f = open("human-eval-events_train.txt", "w")
f.write(str(final_s))
f.close()
